...
- Should return an error (with custom message) if the given line contains the pattern `System.out.println("")`. Otherwise, it should return `null`. Note that:
  ...
  - In your solution, you will likely need to use an escape sequence which are specific sequences of characters in Java to represent special characters in `String`s. In particular, you should use `\"` to insert a double-quote `"` in a `String`.

## Linter Class 📝

Finally, in `Linter.java`, you will implement the constructor and key method of the linter that runs `Check`s on a file.
...
You will likely be able to debug your work more easily if you test each class individually. Note that you **should** be editing `LinterMain` to test all of your `Checks`. Before submitting, make sure your `LinterMain` is testing all of your `Check`s!
...
>BreakCheck
>Description: BreakCheck should return an error (with custom message) if the
>given line contains the break keyword (lowercase) outside of a single line
>comment (comments that start with //). We don't care about the word
>break inside comments, and only in the actual java code.
...
According to the spec, BreakCheck...

>Should return an error (with custom message) if the given line contains the break >keyword outside of a single line comment (comments that start with //); otherwise, it >should return null.
>This check is overly-simplistic in that it might flag some false uses of break such as >`System.out.println("break");`, but you do not need to handle this case specially; you >should flag any use of the keyword break outside of a single line comment.

Here is the reasoning behind each of the answer choices:

- **break;** gets flagged
  - This line contains a break keyword and it is not within a single-line comment, so it should be flagged.
- **System.out.println("good luck on P3!"); // don't break anything :P** is not flagged
  - This line contains the word "break", but it is inside a comment, so it should not be flagged.
- **String break = "break";** gets flagged
  - Although this line doesn't use the forbidden feature break, it still contains the word "break" outside of a single-line comment, so it should be flagged. In the real world, we'd probably want to ignore this case, but we are keeping our linter simple for this assignment.
- **break; // this is a comment** gets flagged
This line contains a `break` keyword and it is not within a single-line comment, so it should be flagged. Notice that a comment can start after some code in the same line.

### Question 2:

All of the statements are true! You can read up on interfaces in Pre-Class Work 14: Interfaces.

- Any class that implements an interface (using the implements keyword) is now under a contract to implement any methods declared in that interface. According to the spec:

>We provided an interface `Check.java` that defines a single method `public Error lint
>(String line, int lineNumber)`. All the checkers you write should implement this >interface and hence, you need to implement the `lint` method in each checker.

- Recall that when declaring objects, it's good style to use the interface type on the lefthand side and the implementation type on the righthand side. This will make the code more flexible!
  - Example: `List<String> = new ArrayList<>(); – List` is the interface type and ArrayList is the class that implements that interface.
  - A client of your code should be able to create a new object with Check as the interface type and any of your three `_____Check` classes as the implementation type.
- According to the spec:

>You should comment your code following Commenting Guide for each file you write. You >should write comments with basic info (a header comment at the top of your file), a >class comment for every class, and a comment for every method other than `main`. In >particular make sure to comment on the interface(s) your classes implement, when >applicable!

### Question 1

When running your BreakCheck class's lint method, which of the following lines should be flagged as an error?

- [ ] break;
- [ ] System.out.println("good luck on P3!"); // don't break anything :P
- [ ] String break = "break";
- [ ] break; // this is a comment

### Question 2

According to the spec, the `BreakCheck`, `LongLineCheck`, and `BlankPrintlnCheck` classes all implement the Check interface. Based on this, which of the following is true for those three classes?

- [ ] They are required to implement the method(s) that are declared in the `Check` interface.
- [ ] They are all types of `Check`'s, so an object declared to have the `Check` interface type (such as `Check check`) could hold any of the three implementation types.
- [ ] Each of their class comments should mention that the class implementation the `Check` interface (in addition to the usual information).
